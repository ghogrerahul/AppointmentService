pipeline {
  agent any
  parameters {
    booleanParam(name: 'SKIP_CI', defaultValue: false, description: 'If true, do not run the CI stages')
    booleanParam(name: 'SKIP_CD', defaultValue: false, description: 'If true, do not run the CD stages')
  }
  environment {
    ...
    stage('CI') {
      ...
      stages {
        ...
        stage('Auto Commit & Push') {
          steps {
            // commit any changes like auto-format or version updates and push back to repo
        ...
        stage('Deploy to ECS') {
          steps {
            withCredentials([usernamePassword(credentialsId: 'aws-creds', passwordVariable: 'AWS_SECRET_ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')]) {
              ...
              SERVICE=${ECS_SERVICE:-appointment-service}
